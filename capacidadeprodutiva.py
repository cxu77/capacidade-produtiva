...
def desc_inv(descontos, psem, monthnumber):
        if descontos[-1] > 0 and calendar.weekday(date.today().year, monthnumber, len(descontos)) % 7 != 6:
            descontos[-1] = -3
            descontos[-2] += -psem
        elif descontos[-1] < 0 and descontos[-1] != -2 and calendar.weekday(date.today().year, monthnumber, len(descontos)) % 7 != 6:
            descontos[-2] = -3
        elif descontos[-1] == -2:
            descontos[-1] = -3
        else:
...
def desc_4turnos(descontos, monthnumber):
    for i in range(len(descontos)):
        if calendar.weekday(date.today().year, monthnumber, i+1) % 7 < 5 and descontos[i] > 0 and descontos[i] !=5:
            descontos[i] -= 1
        elif calendar.weekday(date.today().year, monthnumber, i+1) % 7 >= 5 and descontos[i] > 0 and descontos[i] !=5:
            descontos[i] -= 2
        else:
            k = 0
    return descontos
...
for i in range(0, no_linhas):
    if gap_horas[i] < 0:
        for j in range(len(cal_linhas[0])):
            if calendar.weekday(date.today().year, no_mes, j+1) % 7 == 6:
...
result_agregado.insert(0, 'Linhas', coluna1)
st.write("Gap (em horas e em dias)")
gb = GridOptionsBuilder.from_dataframe(result_agregado)
gb.configure_auto_height()
gb.configure_columns(columns_names =[], groupable=True, value=True, enableRowGroup=True, editable=False)
go = gb.build()
AgGrid(result_agregado, gridOptions = go, enable_enterprise_modules=True)    
